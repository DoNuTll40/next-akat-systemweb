...
export default function Page() {
  ...
  const fetchApi = async () => {
    ...
    try {
      const rs = await axios.get("/setting/getAkathospitalVersionDetails", {
        headers: {
  ...
  const getSelectVersion = async () => {
    ...
    try {
      const rs = await axios.get("/setting/getAkathospitalVersions", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleCreate = async (formData) => {
    ...
    try {
      const rs = await axios.post(
        "/setting/insertAkathospitalVersionDetail",
        { 
          akathospital_version_id: latestVersion.akathospital_version_id,
          akathospital_version_detail_comment: formData.akathospital_version_detail_comment
        },
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleEdit = async (formData) => {
    ...
    try {
      const rs = await axios.put(
        `/setting/updateAkathospitalVersionDetail/${selectedRecord.akathospital_version_detail_id}`,
        { akathospital_version_detail_comment: formData.akathospital_version_detail_comment },
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleDelete = async (record) => {
    ...
    try {
      const rs = await axios.delete(
        `/setting/removeAkathospitalVersionDetail/${record.akathospital_version_detail_id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const columns = [
    ...
    {
      title: "เวอร์ชั่น",
      dataIndex: "akathospital_versions", // ชี้ไปที่ object akathospital_versions
      render: (akathospital_versions) => akathospital_versions?.akathospital_version_name || "-", // แสดง akathospital_version_name หรือ "-" ถ้าไม่มีข้อมูล
      sorter: {
        compare: (a, b) =>
          a.akathospital_versions?.akathospital_version_name.localeCompare(b.akathospital_versions?.akathospital_version_name),
      },
      width: "6rem",
    },
    {
      title: "รายละเอียด",
      dataIndex: "akathospital_version_detail_comment",
      sorter: {
        compare: (a, b) =>
          a.akathospital_version_detail_comment.localeCompare(b.akathospital_version_detail_comment),
      },
      width: "15rem",
    },
    {
  ...
  return (
    <div className="bg-white p-4 rounded-xl shadow-sm select-none">
      ...
      <GenericModal
        ...
        fields={[
          { name: "akathospital_version_id", label: "เวอร์ชั่น", type: "text" },
          { name: "akathospital_version_detail_comment", label: "เวอร์ชั่น", type: "textarea" },
        ]}
        initialData={
          modalMode === "edit" && selectedRecord
            ? {
              akathospital_version_id: selectedRecord?.akathospital_versions?.akathospital_version_name,
              akathospital_version_detail_comment: selectedRecord.akathospital_version_detail_comment,
              }
            : { akathospital_version_id: latestVersion?.akathospital_version_name }
        }
