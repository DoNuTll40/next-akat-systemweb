...
export default function Page() {
  ...
  const handleCreate = async (formData) => {
    ...
    try {
      const rs = await axios.post(
        "/setting//insertAkathospitalVersion",
        { akathospital_version_name: formData.akathospital_version_name },
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleEdit = async (formData) => {
    ...
    try {
      const rs = await axios.put(
        `/setting/updateAkathospitalVersion/${selectedRecord.akathospital_version_id}`,
        { akathospital_version_name: formData.akathospital_version_name },
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleDelete = async (record) => {
    ...
    try {
      const rs = await axios.delete(
        `/setting/removeAkathospitalVersion/${record.akathospital_version_id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const columns = [
    ...
    {
      title: "เวอร์ชั่น",
      dataIndex: "akathospital_version_name",
      sorter: {
        compare: (a, b) =>
          a.akathospital_version_name.localeCompare(b.akathospital_version_name),
      },
      width: "15rem",
    },
    {
  ...
  return (
    <div className="bg-white p-4 rounded-xl shadow-sm select-none">
      ...
      <GenericModal
        ...
        fields={[{ name: "akathospital_version_name", label: "เวอร์ชั่น" }]}
        initialData={
          modalMode === "edit" && selectedRecord
            ? {
              akathospital_version_name: selectedRecord.akathospital_version_name,
              }
            : {}
        }
