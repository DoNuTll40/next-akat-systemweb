...
export default function Page() {
  ...
  const handleCreate = async (formData) => {
    ...
    try {
      const rs = await axios.post(
        "/setting/insertApiVersion",
        { api_version_name: formData.api_version_name },
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleEdit = async (formData) => {
    ...
    try {
      const rs = await axios.put(
        `/setting/updateApiVersion/${selectedRecord.index}`,
        { api_version_name: formData.api_version_name },
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleDelete = async (record) => {
    ...
    try {
      const rs = await axios.delete(
        `/setting/removeApiVersion/${record.api_version_id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const columns = [
    ...
    {
      title: "เวอร์ชั่น",
      dataIndex: "api_version_name",
      sorter: {
        compare: (a, b) =>
          a.api_version_name.localeCompare(b.api_version_name),
      },
      width: "15rem",
    },
    {
  ...
  return (
    <div className="bg-white p-4 rounded-xl shadow-sm select-none">
      ...
      <GenericModal
        ...
        fields={[{ name: "api_version_name", label: "เวอร์ชั่น" }]}
        initialData={
          modalMode === "edit" && selectedRecord
            ? {
              api_version_name: selectedRecord.api_version_name,
              }
            : {}
        }
