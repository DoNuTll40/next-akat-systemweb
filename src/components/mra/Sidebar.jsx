...
export default function MRASidebar() {
  ...
  useEffect(() => {
    ...
    sideMRA.forEach((item, index) => {
      if (item.submenu) {
        const isSubmenuActive = item.submenu.some(sub => pathname.startsWith(sub.path));
  ...
  const activeMenu = sideMRA.reduce((prev, curr) => {
  ...
  return (
    <div
      ...
      style={{ backgroundColor: themeMRA?.sidebarBg, color: themeMRA?.sidebarText }}
    >
      {isOpen && (
              ...
              <div key={index}>
                {item.submenu?.length > 0 ? (
                  <button
                    ...
                    style={{
                      backgroundColor: isActive(item) ? themeMRA?.activeBg : undefined,
                      color: isActive(item) ? themeMRA?.activeText : undefined,
                    }}
                    onClick={(e) => {
                      e.preventDefault();
                    ...
                    onMouseEnter={(e) => {
                      if (!isActive(item)) e.currentTarget.style.backgroundColor = themeMRA?.hoverBg;
                    }}
                    onMouseLeave={(e) => {
                      if (!isActive(item)) e.currentTarget.style.backgroundColor = "";
                    }}
                  >
                  ...
                  <div className="pl-4 border-t rounded-tl-2xl">
                    {Object.entries(
                      ...
                      <div key={gi} className="my-0.5">
                        {group !== "null" && (
                          <div className={`text-sm font-semibold select-none mt-1 ${gi === 0 && "pt-3"}`}>
                            {group}
                          </div>
                        )}
                        {subs.map((sub, si) => (
                          <Link
                            ...
                            style={{
                              backgroundColor: pathname === sub.path ? themeMRA?.activeBg : undefined,
                              color: pathname === sub.path ? themeMRA?.activeText : undefined,
                            }}
                            onMouseEnter={(e) => {
                              if (pathname !== sub.path) e.currentTarget.style.backgroundColor = themeMRA?.hoverBg;
                            }}
                            onMouseLeave={(e) => {
                              if (pathname !== sub.path) e.currentTarget.style.backgroundColor = "";
                            }}
                          >
                            - {sub.name}
                          </Link>
      ...
      {isOpen && (
        <div
          className={`w-full hover:cursor-pointer min-h-8 flex ${isMini ? (onHover ? "justify-end" : "justify-center") : "justify-end"}`}
        ...
        >
          ...
          >
            <ChevronRight
              size={16}
              className={`transition-all transform ease-in-out duration-500 ${isMini ? "rotate-0" : "rotate-180"}`}
            />
          </button>
        </div>
      )}
